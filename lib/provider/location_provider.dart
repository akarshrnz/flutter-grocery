...
class LocationProvider with ChangeNotifier {
  ...
  Position _position = Position(
       altitudeAccuracy: 0,
      headingAccuracy: 0,
    longitude: 0, latitude: 0, timestamp: DateTime.now(), accuracy: 1, altitude: 1, heading: 1, speed: 1, speedAccuracy: 1);
  Position _pickPosition = Position(   altitudeAccuracy: 0,
      headingAccuracy: 0,
  ...
  void getCurrentLocation(BuildContext context, bool fromAddress, {GoogleMapController? mapController}) async {
    ...
    try {
      ...
      myPosition = Position(   altitudeAccuracy: 0,
      headingAccuracy: 0,
  ...
  void updatePosition(CameraPosition? position, bool fromAddress, String? address, bool forceNotify) async {
    if(_updateAddAddressData || forceNotify) {
      ...
      try {
        if (fromAddress) {
          _position = Position(   altitudeAccuracy: 0,
      headingAccuracy: 0,
            latitude: position!.target.latitude, longitude: position.target.longitude, timestamp: DateTime.now(),
            heading: 1, accuracy: 1, altitude: 1, speedAccuracy: 1, speed: 1,
          );
        } else {
          _pickPosition = Position(   altitudeAccuracy: 0,
      headingAccuracy: 0,
            latitude: position!.target.latitude, longitude: position.target.longitude, timestamp: DateTime.now(),
            heading: 1, accuracy: 1, altitude: 1, speedAccuracy: 1, speed: 1,
          );
